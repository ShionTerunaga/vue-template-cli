...
async function binWrite(): Promise<void> {
  ...
  const legacyPackageJson = path.resolve(__dirname, "..", "..", "bin", "package.json");
  const sourcePackageJson = path.resolve(__dirname, "..", "..", "package.json");
  const targetPackageJson = path.resolve(__dirname, "..", "..", "bin", "version.json");
  ...
  const sourcePackageJsonText = await fs.promises.readFile(sourcePackageJson, "utf8");
