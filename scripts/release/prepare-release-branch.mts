...
function isTemporaryRoot(target: string): boolean {
  ...
  const temporaryRoots = [os.tmpdir(), "/tmp"].map((root) => realpathSync(root));
...
async function assertRequiredEntries(): Promise<void> {
  ...
  for (const entry of requiredEntries) {
    const stat = await fs.stat(path.join(repoRoot, entry)).catch(() => null);
...
async function writeReleasePackageJson(): Promise<void> {
  ...
  const sourcePackage = JSON.parse(await fs.readFile(packageJsonPath, "utf8")) as PackageJson;
  ...
  await fs.writeFile(packageJsonPath, `${JSON.stringify(releasePackage, null, 2)}\n`, "utf8");
}
...
async function removeDevelopmentEntries(): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    await fs.rm(path.join(repoRoot, entry), { recursive: true, force: true });
...
async function main(): Promise<void> {
  ...
  if (skipBuild && !configuredRoot) {
    throw new Error("RELEASE_SKIP_BUILD is only supported with a temporary RELEASE_ROOT.");
  }
